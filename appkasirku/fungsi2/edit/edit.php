...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty($_GET['gambar'])) {
        ...
        if(!in_array($filetype, array_keys($allowedTypes))) {
            echo '<script>alert("You can only upload JPG, PNG and GIF file");window.location="../../index2.php?page=user"</script>';
            exit;
            ...
            // echo "<font face='Verdana' size='2' ><BR><BR><BR>
            // 		<a href='../../index.php?page=user'>Back to upform</a><BR>";
            echo '<script>alert("WARNING !!! Besar Gambar Tidak Boleh Lebih Dari 4 MB");window.location="../../index2.php?page=user"</script>';
            exit;
        } else {
            $dir = '../../assets/img/user/';
            ...
            if (move_uploaded_file($tmp_name, $dir.$name)) {
                ...
                echo '<script>alert("Masukan Gambar !");window.location="../../index2.php?page=user"</script>';
                exit;
            }
        }
    }
    ...
    if (!empty($_GET['cari_barang'])) {
		...
		<?php foreach ($hasil1 as $hasil) {?>
			<tr>
				...
				<td>
				<a href="fungsi2/tambah/tambah.php?jual=jual&id=<?php echo $hasil['id_barang'];?>&id_kasir=<?php echo $_SESSION['admin']['id_member'];?>" 
